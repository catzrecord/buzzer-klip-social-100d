#!/usr/bin/env python3
"""Render the complete 100-day queue in the cinematic abstract-human system."""
...
SOURCE_DIR = ROOT / "production-sources" / "abstract-people-v1"
SOURCE_PATHS = [
    SOURCE_DIR / "slide-01.png",
    SOURCE_DIR / "slide-02.png",
    SOURCE_DIR / "slide-03.png",
    SOURCE_DIR / "slide-04.png",
    SOURCE_DIR / "slide-05.png",
    SOURCE_DIR / "single-01.png",
    SOURCE_DIR / "source-07-seated.png",
    SOURCE_DIR / "source-08-duo.png",
    SOURCE_DIR / "source-09-reach.png",
    SOURCE_DIR / "source-10-builder.png",
    SOURCE_DIR / "source-11-ring.png",
    SOURCE_DIR / "source-12-meditate.png",
]
...
def render_asset(item: dict, slide: dict, slide_index: int, sources: list[Image.Image], logo: Image.Image) -> Image.Image:
    item_id = int(item["id"])
    # Co-prime rotation uses every abstract-person source across days and every
    # carousel, instead of collapsing covers into only two repeated poses.
    source_index = (item_id + slide_index * 5) % len(sources)
    ...
    label = item["pillar"] if role in {"cover", "single"} else ROLE_LABELS.get(role, "CREATOR INSIGHT")
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError("Missing abstract-human sources: " + ", ".join(missing))
    ...
    for item in plan:
        for slide_index, slide in enumerate(item["slides"]):
            image = render_asset(item, slide, slide_index, sources, logo)
            save_jpeg(image, ROOT / slide["asset"])
            generated += 1

    print(f"Generated {generated} cinematic abstract-human assets for {len(plan)} posts")
