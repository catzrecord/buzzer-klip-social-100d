...
const rows = plan.map((item) => {
  ...
  return `<article class="${item.status}"><div class="cover"><img src="${item.asset}" alt="${title}"><span>${item.post_type === "carousel" ? `${item.slide_count} SLIDE` : "SINGLE"}</span></div><div><small>#${String(item.id).padStart(3,"0")} · ${item.date} · ${item.post_type} · ${item.content_theme || "archive"} · ${item.subject_family || "legacy"}</small><strong>${title}</strong>${audio}<p>${caption.replaceAll("\n", "<br>")}</p><em>${item.status === "published" ? "LIVE" : "QUEUED"}</em></div></article>`;
}).join("\n");
...
<header><small>BUZZER KLIP · 100-DAY MIXED ABSTRACT QUEUE</small><h1>Banyak Subjek.<br><b>Banyak Cerita.</b></h1><p>${publishedCount} live sekarang · ${queuedCount} berikutnya · ${plan.length} hari · ${blankCount} caption kosong · manusia, objek, dan scene abstrak · edukasi, humor, komunitas, cerita, challenge, brand, culture, dan motivasi</p></header>
